...
import (
	...
	"github.com/longerfly/cubefs/blobstore/api/access"
	"github.com/longerfly/cubefs/blobstore/common/trace"
	"github.com/longerfly/cubefs/blockcache/bcache"
	"github.com/longerfly/cubefs/client/fs"
	"github.com/longerfly/cubefs/proto"
	"github.com/longerfly/cubefs/sdk/data/blobstore"
	"github.com/longerfly/cubefs/sdk/data/stream"
	masterSDK "github.com/longerfly/cubefs/sdk/master"
	"github.com/longerfly/cubefs/sdk/meta"
	"github.com/longerfly/cubefs/util/auditlog"
	"github.com/longerfly/cubefs/util/buf"
	"github.com/longerfly/cubefs/util/errors"
	"github.com/longerfly/cubefs/util/log"
	"github.com/longerfly/cubefs/util/stat"
)
...
func (c *Client) Rename(from, to string, overwritten bool) error {
	...
	err = c.mw.Rename_ll(srcDirInfo.Inode, srcName, dstDirInfo.Inode, dstName, overwritten)
...
func (c *Client) allocFD(ino uint64, flags int, mode uint32, fileCache bool, fileSize uint64, parentInode uint64) *File {
	...
	c.fdset.Set(fd)
	f := &File{client: c, fd: fd, ino: ino, flags: flags, mode: mode, pino: parentInode}
...
func (c *Client) SymLink(srcPath, dstPath string) error {
	...
	info, err = c.mw.Create_ll(parentIno, filename, proto.Mode(os.ModeSymlink|os.ModePerm), 0, 0, []byte(fullSrcPath))
	if err != nil {
...
func (c *Client) Link(srcPath, dstPath string) error {
	...
	info, err = c.mw.Link(parentIno, filename, srcIno)
	if err != nil {
